...
import { addShop, deleteShop } from '../src/data/repository/customerRepository';
...
export default function ShopsScreen() {
  ...
  async function handleAdd() {
    if (!user) {
      Alert.alert('تعذر الإضافة', 'سجّل الدخول أولًا');
      return;
    }
    if (!name.trim()) {
      Alert.alert('تنبيه', 'أدخل اسم المحل');
      return;
    }
    ...
    try {
      ...
      Alert.alert('خطأ', e.message || 'تعذر إضافة المحل');
    } finally {
      setSaving(false);
    }
  }
          ...
          return (
            <Pressable key={shop.id} style={styles.card} onPress={() => router.push(`/shop/${shop.id}`)}>
              ...
              <Text style={styles.cardMeta}>
                {shop.category ? `${shop.category} • ` : ''}{count} صنف
              </Text>
            </Pressable>
          );
        })
